...
import { followService, postService, userService } from "@/services";
...
const [userData, userPosts, likedPostsData, followers, following] = await Promise.all([
        userService.getCurrentUser(),
        postService.getByUserId(currentUserId),
        postService.getLikedPosts(currentUserId),
...
const handlePostCreated = async (newPost) => {
  ...
  if (loading) {
    ...
    <div className="container mx-auto px-4 py-6 max-w-4xl">
        ...
        <div
            className="bg-surface rounded-xl border border-surface-200 p-6 mb-6 animate-pulse">
            <div
        ...
        <motion.div
            ...
            <div
                ...
                <motion.div
                    ...
                    className="relative">
                    <Avatar src={user?.avatar} alt={user?.displayName} size="3xl" hasStory={true} />
                    <button
                ...
                <div className="flex-1">
                    <div
                        ...
                        <div className="flex space-x-2 mt-4 sm:mt-0">
                            <Button variant="primary" size="sm" onClick={() => setShowCreateModal(true)}>
                                <ApperIcon name="Plus" className="w-4 h-4 mr-2" />New Post
                                                  </Button>
                    ...
                    {/* Bio */}
                    {user?.bio && <p className="text-surface-700 mb-4 break-words">
                        {user.bio}
                    </p>}
                    {/* Stats */}
        ...
        <motion.div
            ...
            <div className="flex border-b border-surface-200">
                {[{
                    ...
                    label: "Media",
                    icon: "Image",
                    count: posts.filter(p => p.mediaUrls?.length > 0).length
                }, {
                    id: "likes",
                    label: "Likes",
                    icon: "Heart",
                    ...
                    className={`flex-1 px-4 py-3 text-sm font-medium transition-colors duration-200 flex items-center justify-center space-x-2 ${activeTab === tab.id ? "text-primary border-b-2 border-primary bg-primary/5" : "text-surface-600 hover:text-surface-900 hover:bg-surface-50"}`}>
                    <ApperIcon name={tab.icon} className="w-4 h-4" />
                    <span>{tab.label}</span>
                    <span
                        className="text-xs bg-surface-200 text-surface-600 px-2 py-0.5 rounded-full">
                        {tab.count}
                    </span>
                </button>)}
            </div>
        ...
        <div>
            {activeTab === "posts" && <div className="space-y-6">
                {posts.length === 0 ? <EmptyState
                    ...
                    onAction={() => setShowCreateModal(true)} /> : posts.map((post, index) => <motion.div
                    key={post.id}
                    initial={{
                        opacity: 0,
                        y: 20
                    ...
                    <PostCard post={post} user={user} onUpdate={handlePostUpdate} />
                </motion.div>)}
            </div>}
            {activeTab === "media" && <div>
                {posts.filter(p => p.mediaUrls?.length > 0).length === 0 ? <EmptyState
                    icon="Image"
                    title="No media posts"
                    description="Share photos and videos to see them here"
                    actionLabel="Upload Media"
                    onAction={() => setShowCreateModal(true)} /> : <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                    {posts.filter(post => post.mediaUrls?.length > 0).map((post, index) => <motion.div
                        key={post.id}
                        initial={{
                            opacity: 0,
                            scale: 0.9
            ...
            {activeTab === "likes" && <div className="space-y-6">
                {likedPosts.length === 0 ? <EmptyState
                    icon="Heart"
                    title="No liked posts yet"
                    description="Posts you've liked will appear here" /> : likedPosts.map((post, index) => <motion.div
                    key={post.id}
                    initial={{
                        opacity: 0,
                        y: 20
